        ...
        h(
          NInputNumber, {
            ...
            onBlur() {
              ...
              const amount = Number(tableData.value[index].eachAmount) - (Number(tableData.value[index].commission) || 0.00)

              tableData.value[index].actualAmount = amount < 0 ? `-${keepTwoDecimalStr(Math.abs(amount))}` : keepTwoDecimalStr(amount)

              tableData.value.forEach((item: any) => {
                if (item.eachAmount)
                  num = num + item.eachAmount * 1
              })

              tableData.value.forEach((item: any) => {
                if (item.actualAmount)
                  amountNum = amountNum + item.actualAmount * 1
              })

              Data.mainData.budPayMoney = num < 0 ? `-${keepTwoDecimalStr(Math.abs(num))}` : keepTwoDecimalStr(num)

              Data.mainData.actualAmountSum = amountNum < 0 ? `-${keepTwoDecimalStr(Math.abs(amountNum))}` : keepTwoDecimalStr(amountNum)
            },
      ...
      return [
        h(
          NInputNumber, {
            ...
            clearable: true,
            onUpdateValue(v) {
              tableData.value[index].commission = v
            },
            onBlur() {
              let num = 0
              let amountNum = 0
              const val = 0
              const toFixedNum = tableData.value[index].commission?.toFixed(3)
              tableData.value[index].commission = toFixedNum?.substring(0, toFixedNum.lastIndexOf('.') + 3) || val.toFixed(3).substring(0, val.toFixed(3).lastIndexOf('.') + 3)
              const amount = (tableData.value[index].eachAmount || 0.00) - tableData.value[index].commission
              tableData.value[index].actualAmount = amount < 0 ? `-${keepTwoDecimalStr(Math.abs(amount))}` : keepTwoDecimalStr(amount)
              tableData.value.forEach((item: any) => {
                if (item.commission)
                  num = num + item.commission * 1
              })
              ...
              Data.mainData.commissionSum = num < 0 ? `-${keepTwoDecimalStr(Math.abs(num))}` : keepTwoDecimalStr(num)
  ...
  if (type === 1) {
    formRef.value?.validate(async (errors) => {
      if (!errors) {
        ...
        Data.mainData.feeType = 1
        let num = 0
        let num1 = 0
        let num2 = 0
        tableData.value.forEach((item) => {
          item.eachAmount = item.eachAmount || keepTwoDecimalStr(0)
          item.commission = item.commission || keepTwoDecimalStr(0)
          item.actualAmount = item.actualAmount || keepTwoDecimalStr(item.eachAmount - item.commission)
          num = num + (item.eachAmount || keepTwoDecimalStr(0)) * 1
          num1 = num1 + (item.commission || keepTwoDecimalStr(0)) * 1
          num2 = num2 + (item.actualAmount || keepTwoDecimalStr(0)) * 1
        })
        Data.mainData.budPayMoney = num < 0 ? `-${keepTwoDecimalStr(Math.abs(num))}` : keepTwoDecimalStr(num)
        Data.mainData.commissionSum = num1 < 0 ? `-${keepTwoDecimalStr(Math.abs(num1))}` : keepTwoDecimalStr(num1)
        Data.mainData.actualAmountSum = num2 < 0 ? `-${keepTwoDecimalStr(Math.abs(num2))}` : keepTwoDecimalStr(num2)
        const res
          = !Data.editFlag
...
async function selectBudCompile() {
  const res = await payRegApi.selectBudCompile({ feeType: 1, assetId: Data.mainData.assetId, branchId: Data.mainData.branchId, deptId: Data.mainData.deptId })
  if (res.code === 200)
...
async function keyCheck() {
  if (Data.mainData.budControlPayCode) {
    const res = await payRegApi.selectIfExist({ code: Data.mainData.budControlPayCode, type: 4, feeType: 1, assetId: Data.mainData.assetId ?? '' })
    if (res.code === 200)
...
async function fetchCode() {
  const res = await payRegApi.fetchCode({ type: 4, feeType: 1, assetId: Data.mainData.assetId ?? '' })
  if (res.code === 200)
...
<template>
  <n-modal
    ...
    :title="`收入登记${!Data.editFlag ? '录入' : '编辑'}`" size="huge"
  ...
  >
        ...
        <n-grid :cols="24" :x-gap="24">
          ...
          <n-form-item-gi label="银行账号" span="6" path="bankAccountNumber">
            <NSelect
              ...
              :disabled="Data.mainData.ifCorrelation === '0'" @click="selectListByBankType()"
          ...
          <n-form-item-gi label="收入总金额" span="6" path="budPayMoney">
            <NInput
    ...
    <n-card mt-10>
      ...
      <n-data-table
        :style="{ minHeight: `${tHeight - 600}px` }"
