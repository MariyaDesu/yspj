...
async function handleDelete(row: RowData) {
  ...
  if (row.state !== 0 && row.state !== 5) {
    window.$message?.error('调整登记状态-编制中才可删除')
    return
  }
...
function handleEdit(row: RowData) {
  if (row.state !== 0 && row.state !== 5) {
    window.$message?.error('调整登记状态-编制中、未通过才可编辑')
    return
  }
...
<template>
  <vxe-table ref="xTable" border :tree-config="{
    ...
    <vxe-column field="budControlPayCode" title="调整编号" width="200" />
    <vxe-column field="assetName" title="产权结构" width="300" />
    <vxe-column field="deptName" title="部门" width="300" />
    <vxe-column field="abstracts" title="摘要" width="200" />
    <vxe-column field="budPayMoney" title="总金额" width="200" />
