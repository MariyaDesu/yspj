  ...
  if (type === 1) {
    formRef.value?.validate(async (errors) => {
      if (!errors) {
        ...
        Data.mainData.budPayMoney = num < 0 ? `-${keepTwoDecimalStr(Math.abs(num))}` : keepTwoDecimalStr(num)
        if (Number(Data.mainData.budPayMoney) !== 0) {
          window.$message?.error('请确认总金额为0')
          return
        }
        const res
          = !Data.editFlag
...
async function selectBudCompile() {
  const res = await payRegApi.selectBudCompile({ feeType: 3, assetId: Data.mainData.assetId, branchId: Data.mainData.branchId, deptId: Data.mainData.deptId })
  if (res.code === 200)
...
async function keyCheck() {
  if (Data.mainData.budControlPayCode) {
    const res = await payRegApi.selectIfExist({ code: Data.mainData.budControlPayCode, type: 5, feeType: 3, assetId: Data.mainData.assetId ?? '' })
    if (res.code === 200)
...
async function fetchCode() {
  const res = await payRegApi.fetchCode({ type: 5, feeType: 3, assetId: Data.mainData.assetId ?? '' })
  if (res.code === 200)
...
<template>
  <n-modal
    ...
    :title="`调整登记${!Data.editFlag ? '录入' : '编辑'}`" size="huge"
    ...
    <n-card title="基本数据">
      ...
      >
        <n-grid :cols="24" :x-gap="24">
          <n-form-item-gi label="调整编号" span="6" path="budControlPayCode">
            <NInput
              v-model:value="Data.mainData.budControlPayCode" clearable placeholder="请输入" :disabled="Data.editFlag"
              @blur="keyCheck"
            />
          </n-form-item-gi>
          <n-form-item-gi label="调整方式" span="6" path="payType">
            <NSelect
          ...
          <n-form-item-gi label="调整日期" span="6" path="payTime">
            <n-date-picker
