...
function fetchTableData() {
  instance?.proxy?.$Bus.emit('fetchBudCtlTableData', { isAffairs: Datas.isAffairs })
}
...
instance?.proxy?.$Bus.off('changeBudCtlAppAffairs')
instance?.proxy?.$Bus.off('editBudCtl')

onMounted(() => {
  instance?.proxy?.$Bus.on('changeBudCtlAppAffairs', (value: any) => {
...
onMounted(() => {
  instance?.proxy?.$Bus.on('editBudCtl', async (row: any) => {
    // 编辑
    loading.value = true
  ...
  >
    <n-layout has-sider :style="{ minHeight: `${tHeight}px` }">
      <n-layout-content :native-scrollbar="false" mr-20>
        ...
        <n-card mt-10>
          <n-data-table
            :style="{ minHeight: `${tHeight - 400}px` }" flex-height
