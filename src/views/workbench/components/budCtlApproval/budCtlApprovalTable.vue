...
async function handleDetails(row: RowData) {
  instance?.proxy?.$Bus.emit('changeBudCtlAppAffairs', { row, isAffairs: Data.isAffairs })
  instance?.proxy?.$Bus.emit('editBudCtl', { row, isAffairs: Data.isAffairs })
}
...
instance?.proxy?.$Bus.off('fetchBudCtlTableData')

instance?.proxy?.$Bus.on('fetchBudCtlTableData', (row: { isAffairs: boolean }) => {
...
async function initTableData() {
  ...
  try {
    ...
    const res = Data.isAffairs ? await workBenchApi.getListControlByAdmin({ mStatus: 0 }) : await workBenchApi.getListControlByAdmin({ mStatus: 1 })
    if (res.code === 200) {
    ...
    >
      ...
      <vxe-column field="status" title="状态" width="" :min-width="200">
        <template #default="scope">
          {{ scope.row.status === 0 ? '审批中' : scope.row.status === 1 ? '已完结' : scope.row.status === 2 ? '未通过' : '' }}
        </template>
      </vxe-column>

      <vxe-table-column title="操作" width="120" show-overflow align="center" fixed="right">
        <template #default="scope">
          <NButton type="info" ml-10 mr-10 size="small" @click="handleDetails(scope.row)">
            查看详情
          </NButton>
        </template>
      </vxe-table-column>
