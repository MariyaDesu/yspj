...
const columns: any = reactive({
  tableColumns: [
    { title: '是否有合同', field: 'contractStateName', align: 'left', width: 120 },
...
const Data: IData = reactive({
  ...
  params: {
    pageNum: 0,
    pageSize: 0,
    beginTimeYear: (new Date().getFullYear() - 2).toString(),
...
async function initTitleData() {
  loading.value = true
  try {
    const res = await budgetReportApi.fetchGetOutsideTitle(Data.params)
    if (res.code === 200) {
      Data.sheetData = []
...
async function initTableData() {
  loading.value = true
  try {
    const res = await budgetReportApi.fetchNowAndPreviousYears(Data.params)
    Data.sheetTitle = []
    ...
    <vxe-column type="expand" title="" width="80" fixed="left">
      <template #content="{ row }">
        ...
        >
          ...
          <vxe-column field="month" title="月份" width="60" fixed="left" align="left" />
          <template v-for="item in columns.titleColumns" :key="item.key">
            <vxe-column :field="item.field" :title="item.title" formatter="formatAmount" align="right" />
          </template>
        </vxe-table>
      </template>
    </vxe-column>
