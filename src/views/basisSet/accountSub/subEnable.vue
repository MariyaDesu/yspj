...
const columns: DataTableColumns<RowData> = [
  { type: 'selection' },
  { title: '会计科目代码', key: 'code', width: 80, align: 'center', ellipsis: { tooltip: true } },
  { title: '会计科目名称', key: 'name', width: 80, align: 'center', ellipsis: { tooltip: true } },
  {
    title: '余额方向',
...
async function initTableData() {
  loading.value = true
  try {
    const res = await accountSubApi.budgetEntryListDisable({ code: search.code, name: search.name })
    if (res.code === 200)
...
async function handleEnable(keyArray: Array<number>) {
  window.$dialog?.warning({
    ...
    onPositiveClick: async () => {
      const res = await accountSubApi.removeBudEntry({ list: keyArray, flag: 0 })
      if (res.code === 200) {
    ...
    <n-layout pl-10>
      ...
      <n-layout mt-10>
        ...
        <n-pagination
          ...
          v-model:page-size="Data.page.pageSize" mt-20 :page-count="Data.page.totalPage"
          size="large"
          show-quick-jumper
          show-size-picker
