...
async function editMsg() {
  formRef.value?.validate(async (errors) => {
    if (!errors) {
      ...
      // 确保用户直接点保存,不填值,系统默认给0.00,
      // Data.mainData.budCompileList为外层预算数据
      // Data.mainData.budCompileList[i].budCompileDataList为里层详情数据
      ...
      const res
        = JSON.stringify(Data.editData) === '{}'
          ? await budgetCompileListApi.saveAddBudCompileListEdit(Data.mainData)
...
async function keyCheck() {
  if (Data.mainData.compileCode) {
    const res = await budgetCompileListApi.selectIfExist({ code: Data.mainData.compileCode, type: 1, assetId: Data.mainData.assetId ?? '' })
    if (res.code === 200)
...
async function fetchCode() {
  const res = await budgetCompileListApi.fetchCode({ type: 1, assetId: Data.mainData.assetId ?? '' })
  // options.branchOptions = res.data
  if (res.code === 200)
    Data.mainData.compileCode = res.data
}
// 获取部门
async function fetchDept(val: string | undefined) {
  if (!val)
    return
...
<template>
  <n-modal
    ...
    :title="`预算数据${JSON.stringify(Data.editData) === '{}' ? '录入' : '编辑'}`" size="huge"
  >
    <n-card title="基本数据">
      <n-form
      ...
      >
        <n-grid :cols="24" :x-gap="24">
          <n-form-item-gi label="预算编制代码" span="6" path="compileCode">
            <NInput v-model:value="Data.mainData.compileCode" clearable placeholder="请输入" @blur="keyCheck" />
          </n-form-item-gi>
