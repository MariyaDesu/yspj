...
// 初始化edit/detailList数据
function initList(row: unknown) {
  instance?.proxy?.$Bus.emit('initList', row)
}
function handleEdit(row: RowData, index: number) {
  Data.keyIndex = index
...
// 临时存放的数据
function handleAdjust(row: RowData, index: number) {
...
// 判断审批成功/失败图片是否显示
function isPassed(step, type: number) {
  if (type === 1) {
    if (step.status === 1)
      return true
    return false
...
instance?.proxy?.$Bus.on('editType2', async (row: any) => {
  // 编辑
  loading.value = true
...
instance?.proxy?.$Bus.on('processType2', (value: any) => {
  Datas.affairs = value.affairs
  if (value.row)
...
<template>
  ...
  >
    ...
    <n-card v-if="Datas.affairs && (Datas.mainData.status !== 1 && Datas.mainData.status !== 2)" title="审批意见" flex-shrink-0 mt-10 cursor-pointer>
      <NInput
